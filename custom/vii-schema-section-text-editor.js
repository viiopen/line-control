	...
	var methods = {
		...
		init : function( options )
		{
							 ...
							 'font_size': {	"select":true,
							  ...
							  'color'	: { "text":"A",
											...
											"custom":function(button){
													...
													var paletteCntr   = $('<div/>',{id:"paletteCntr",class:"activeColour", css :{"display":"none"}}).click(function(event){event.stopPropagation();});
                          var paletteSubCntr = $('<div/>', {id:"paletteSubCntr"});
													var paletteDiv    = $('<div/>',{id:"colorpellete"}).html('<span>Text</span>');
													//var palette       = $('<ul />',{id:"color_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Text Color</div>'));
													var palette       = $('<ul />',{id:"color_ui"});
													var bgPalletteDiv = $('<div/>',{id:"bg_colorpellete"}).html('<span>Background</span>');
													//var bgPallette    = $('<ul />',{id:"bgcolor_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Background Color</div>'));
													var bgPallette    = $('<ul />',{id:"bgcolor_ui"});
													if(editor.data("colorBtn") && $('#paletteSubCntr').length > 0){
														flag = 1;
													...
													if(flag==0){
														...
														paletteDiv.appendTo(paletteSubCntr);
														bgPalletteDiv.appendTo(paletteSubCntr);
														paletteSubCntr.appendTo(paletteCntr);
														paletteCntr.insertAfter(button);
														//$('#paletteCntr').slideDown('slow');
														$('#paletteCntr').slideDown('fast');
													}
													else
														$('#paletteCntr').slideUp().remove();
												}},
						   ...
						   'outdent'	: { "text": "Outdent",
							...
							'clear': { "text": "Clear",
											...
											"custom":function(){
												...
												$(this).data("editor").children().remove();
											}},
			...
			var menuGroups = {'texteffects' : ['bold', 'italics', 'underline', 'color'],
							  ...
							  'advancedoptions' : ['print', 'rm_format', 'select_all', 'clear', 'source'],
...
console.log("VIEW", html);
		...
		setStyleWithCSS:function(){
			if(navigator.userAgent.match(/MSIE/i)){	//for IE10
				try {
                	ViiSchemaSectionTextEditor.execCommand("styleWithCSS", 0, false);
            	} catch (e) {
	                try {
	                    ViiSchemaSectionTextEditor.execCommand("useCSS", 0, true);
	                } catch (e) {
	                    try {
	                        ViiSchemaSectionTextEditor.execCommand('styleWithCSS', false, false);
	                    }
	                    catch (e) {
	                    }
	                }
	...
	$.fn.ViiSchemaSectionTextEditor = function( method ){
